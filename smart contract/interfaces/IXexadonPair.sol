...
interface IXexadonPair {
    ...
    function addLiquidity(uint256[] memory tokenIds, address _from) external payable returns (bool);
    // function removeLiquidity(uint256[] memory tokenIds, address _to, uint256 _shares) external returns (bool);
    function swap(uint256[] memory tokenIds, address to, bool tokenIn) external payable returns(bool);
